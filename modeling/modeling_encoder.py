...
from transformers import (OPENAI_GPT_PRETRAINED_CONFIG_ARCHIVE_MAP, BERT_PRETRAINED_CONFIG_ARCHIVE_MAP,
                          ...
                          ALBERT_PRETRAINED_CONFIG_ARCHIVE_MAP, DISTILBERT_PRETRAINED_CONFIG_ARCHIVE_MAP)
...
MODEL_CLASS_TO_NAME = {
    ...
    'distilbert': list(DISTILBERT_PRETRAINED_CONFIG_ARCHIVE_MAP.keys()),
...
class TextEncoder(nn.Module):
    ...
    def __init__(self, model_name, output_token_states=False, from_checkpoint=None):
        super().__init__()
        self.model_type = MODEL_NAME_TO_CLASS.get(model_name, model_name)
        ...
        # else:
        self.module = AutoModel.from_pretrained(model_name.replace("___", "/"), output_hidden_states=True)
    ...
    def forward(self, *inputs, layer_id=-1):
        ...
        if model_type in ('gpt',):  # gpt
            input_ids, cls_token_ids, lm_labels = inputs  # lm_labels is not used
            outputs = self.module(input_ids)
        elif model_type in ('distilbert',) or 'minilm' in model_type or 'roberta' in model_type:
        ...
        elif self.model_type in ('albert', 'distilbert'):
