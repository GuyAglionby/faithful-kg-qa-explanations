...
def add_data_arguments(parser):
    ...
    parser.add_argument('--ent_emb', default=[], choices=['tzw'], nargs='+', help='sources for entity embeddings')
    parser.add_argument('--graph', default='cpnet', type=str, help='Which graph to use')
    # dataset specific
    ...
    parser.add_argument('--inhouse_train_qids', default='./data/{dataset}-{graph}/inhouse_split_qids.txt', help='qids of the in-house training set')
    # statements
    parser.add_argument('--train_statements', default='./data/{dataset}-{graph}/statement/train.statement.jsonl')
    parser.add_argument('--dev_statements', default='./data/{dataset}-{graph}/statement/dev.statement.jsonl')
    parser.add_argument('--test_statements', default='./data/{dataset}-{graph}/statement/test.statement.jsonl')
    ...
    parser.set_defaults(ent_emb_paths=[EMB_PATHS.get(s) for s in args.ent_emb],
                        ...
                        inhouse_train_qids=args.inhouse_train_qids.format(dataset=args.dataset, graph=args.graph))
    ...
    for split in data_splits:
        for attribute in ('statements',):
            ...
            parser.set_defaults(**{attr_name: getattr(args, attr_name).format(dataset=args.dataset, graph=args.graph)})
...
def add_additional_arguments(parser):
    ...
    if args.debug:
        ...
        parser.set_defaults(log_interval=5, eval_interval=5)
