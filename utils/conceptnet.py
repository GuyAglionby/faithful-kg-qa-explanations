...
def extract_english(conceptnet_path, output_csv_path, output_vocab_path):
    ...
    :return:
    """
    if os.path.exists(output_csv_path) and os.path.exists(output_vocab_path):
        print('concept english relations already extracted')
        return
...
def construct_graph(cpnet_csv_path, cpnet_vocab_path, output_path, graph, prune=True):
    if os.path.exists(output_path):
        print('conceptnet graph file already exists')
        return
    ...
    with open(cpnet_csv_path, "r", encoding="utf8") as fin:
        ...
        for ls in tqdm(csv.reader(fin, delimiter='\t'), total=nrow):
            if any(cell.strip() == '' for cell in ls[:3]):
                print(f"{ls} has an empty element; skipping")
                continue
            ...
            if (subj, obj, rel) not in attrs:
                nx_graph.add_edge(subj, obj, rel=rel, weight=weight)
                attrs.add((subj, obj, rel))
                nx_graph.add_edge(obj, subj, rel=rel + len(relation2id), weight=weight)
