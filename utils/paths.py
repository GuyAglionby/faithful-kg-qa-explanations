...
def find_paths_qa_concept_pair(source: str, target: str, graph, ifprint=False):
    """
    ...
    try:
        for p in nx.shortest_simple_paths(cpnet_simple, source=s, target=t):
            if len(p) > MAX_PATH_LEN or len(all_path) >= 100:  # top 100 paths
                break
            ...
            if qc not in simple_schema_graph.nodes() or ac not in simple_schema_graph.nodes():
                ...
                try:
                    for p in nx.shortest_simple_paths(simple_schema_graph, source=qc, target=ac):
                        if len(p) >= MAX_PATH_LEN or len(all_path) > 100:
                            break
                        if len(p) >= 2:  # skip paths of length 1
                            all_path.append(p)
                except nx.exception.NetworkXNoPath:
...
def find_paths_qa_pair(graph, qa_pair):
    acs, qcs = qa_pair
    pfr_qa = []
    for ac in acs:
        for qc in qcs:
            pf_res = find_paths_qa_concept_pair(qc, ac, graph)
            pfr_qa.append({"ac": ac, "qc": qc, "pf_res": pf_res})
    return pfr_qa
...
def score_triples(concept_id, relation_id, graph, debug=False):
    global relation_embs, concept_embs, id2relation, id2concept
    concept = concept_embs[concept_id]
    relation = []
    flag = []
    n_relations = relation_embs.shape[0]
    for i in range(len(relation_id)):
        embs = []
        l_flag = []

        if graph == 'cpnet':
            # This is for symmetrical relations: relatedto (0) and antonym (15)
            # The reverse relations for everything are always included anyway - as `x + num_relations` for rel id x
        ...
        for j in range(len(relation_id[i])):
            if relation_id[i][j] >= n_relations:
                ...
                # print("graph", graph, "relation_id")
                # print(relation_id)
                # print("i", i, "j", j)
                # print(relation_id[i][j])
                # print("relation_embs", relation_embs.shape)
...
def score_qa_pairs(graph, qa_pairs_str):
    ...
    for qas in qa_pairs:
        ...
        if statement_paths is not None:
            ...
            for path in statement_paths:
                ...
                score = score_triples(concept_id=path["path"], relation_id=path["rel"], graph=graph)
...
def find_paths(grounded_path, cpnet_vocab_path, cpnet_graph_path, output_path, graph, num_processes=1, random_state=0):
    if os.path.exists(output_path):
        print(f'path already exists for {grounded_path}')
        return
    set_max_path_len(graph)
    ...
    with Pool(num_processes) as p, open(output_path, 'w') as fout:
        find_paths_qa_pair_partial = partial(find_paths_qa_pair, graph)
        for pfr_qa in tqdm(p.imap(find_paths_qa_pair_partial, data), total=len(data)):
...
def generate_path_and_graph_from_adj(adj_path, cpnet_graph_path, output_path, graph_output_path, graph, num_processes=1, random_state=0, dump_len=False):
    if os.path.exists(output_path) and os.path.exists(graph_output_path):
        print(f'paths already exist for {adj_path}')
        return
    set_max_path_len(graph)
...
def score_paths(raw_paths_path, concept_emb_path, rel_emb_path, cpnet_vocab_path, output_path, graph, num_processes=1, method='triple_cls'):
    if os.path.exists(output_path):
        print(f'paths already exist for {raw_paths_path}...')
        return
    set_max_path_len(graph)
    ...
    print(f"Loaded concept embeddings {concept_embs.shape} relation embeddings {relation_embs.shape}")
    ...
    with Pool(num_processes) as p, open(output_path, 'w') as fout:
        score_qa_pairs_partial = partial(score_qa_pairs, graph)
        for statement_scores in tqdm(p.imap(score_qa_pairs_partial, data), total=len(data)):
...
def prune_paths(raw_paths_path, path_scores_path, output_path, threshold, graph, verbose=True):
    if os.path.exists(output_path):
        print(f'Paths already exist for {raw_paths_path}')
        return
    set_max_path_len(graph)
...
def find_relational_paths_from_paths(pruned_paths_path, output_path, num_processes):
    if os.path.exists(output_path):
        print(f'relational paths already exist for {pruned_paths_path}')
        return
